...
int cthread_create(cthread *c, void *(*f)(void *), void *args)
{
    ...
    c->tid = clone(cthread_run, stackhead, CLONE_VM | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM | CLONE_SIGHAND | CLONE_THREAD | CLONE_PARENT_SETTID | CLONE_CHILD_CLEARTID, c, &c->futex_val, c, &c->futex_val);
